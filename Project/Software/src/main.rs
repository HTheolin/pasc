...
const APP: () = {
    ...
    fn start_timer(){
        ...
        if resources.COUNTDOWNTIMER.get_startT() > 0{
            ...
            // resources.pwm.set_duty(*pwm.c, pwm.get_max_duty() / 2);
            // resources.pwm.enable(*pwm.c);
            // schedule.screech(Instant::now() + (3*SECOND).cycles()).unwrap();
        }  
    }
