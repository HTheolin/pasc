...
use hal::stm32::{ADC1, DMA2, RCC, TIM1, TIM2, TIM3, TIM4, GPIOA, GPIOB, GPIOC};
...
pub struct Adc<'a, T>(pub &'a ADC1, pub &'a T)
where 
    T: 'a;
    ...
    ($TIM:ident) => { 
        impl<'a> Adc<'a, $TIM> 
        {
            ...
            pub fn init(&self, dma2: &DMA2, rcc: &RCC) {
                ...
                let mut tim_bits = 0b0000;
                if tim.type_id() == TypeId::of::<TIM1>() {
                     tim_bits = 0b0001; //T1C2
                } else if tim.type_id() == TypeId::of::<TIM2>() {
                     tim_bits = 0b0011; //T2C2
                } else if tim.type_id() == TypeId::of::<TIM3>() {
                    tim_bits = 0b0111;  //T3C1
                } else if tim.type_id() == TypeId::of::<TIM4>() {
                     tim_bits = 0b1001; //T4C4
                }
            ...
            /// triggered by the channel 2 of TIM1 and TIM2, channel 1 on TIM3, channel 4 on TIM4
            ///
                ...
                B: Unsize<[u16]>,
            {
                ...
                let mut c = Channel::_2; //TIM1 & TIM2
                if tim.type_id() == TypeId::of::<TIM3>() {
                    c = Channel::_1;  //TIM3 C1
                } else if tim.type_id() == TypeId::of::<TIM4>() {
                    c = Channel::_4; //TIM4 C4
                }
                pwm.disable(c);
                pwm.set_duty(c, 1);
