...
impl Temperature {
    ...
    pub fn read(&mut self) -> f32 {
        self.temp = Self::to_celsius(self.sample);
        return self.temp;
    }

    fn to_celsius(sample: u16) -> f32 {
        let volts: f32 = Self::bits_to_volts(sample);
        let ohms: f32 = Self::volts_to_ohms(volts);
        let temp: f32 = Self::ohms_to_celsius(ohms);

        let decimals: u8 = 1;
        let temp_trunc: f32 = Self::truncate_f32(temp, decimals);
        return temp_trunc;
    }
    ...
    fn ohms_to_celsius(r: f32) -> f32 {
        ...
        if r < r0 {
            ...
            while tolerance < dt {
                let tn: f32 = t - (Self::t2r(t) - r) / Self::t2r_prime(t);
                dt = tn - t;
                t = tn;
            }
        }
